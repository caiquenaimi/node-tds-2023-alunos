...
export const getUsers = async (req, res) => {
  try {
    ...
    return res.status(200).send({ message: "Total de usuários encontrados : " + users.length, users });
  } catch (error) {
...
export const getUserById = async (req, res) => {
  try {
    ...
    return res.status(200).send({ message: `Usuário com id : ${id}`, user });
  } catch (error) {
    return res.status(500).send({ message: "Erro ao encontrar o usuário ", error: error.message });
  }
};
